#!/usr/bin/env bash
# run.sh — the turnkey runner for the "k8s: deploy an app" demo (ADR-0116 D2). One command drives the
# whole descent from a genesis-floor Stratt: apply the demo estate, launch the gated deploy-hello
# Workflow, approve the platform-admins gate as the dev-header bootstrap-admin Principal, wait for the
# WorkflowRun to converge, and ASSERT the real `hello` Deployment is Ready. It reuses the proven
# genesis-selfdeploy launch/approve/assert loop — the turnkey keeps the narrative (README.md) honest.
#
# Assumes `task dev:genesis` already stood up the minimal Stratt (helm Actuator + helm-deploy
# CredentialRef registered). Invoked by `task demo:k8s-deploy:run`. Needs curl + jq + kubectl + go.
#
...
# ── Apply the demo estate (adds the deploy-hello Workflow + hello View on top of genesis) ─────────
echo "demo: apply demos/k8s-deploy/estate (stratt apply)"
(cd "$ROOT_REPO" && go run ./core/cmd/stratt apply -d "${HERE}/estate" -s "$ROOT")
