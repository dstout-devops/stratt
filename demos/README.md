...
| **[EC2: provision a real instance](../plugins/awsec2/demo/README.md)**            | `plugins/awsec2/`                       | EC2 (floci)              | `build-real`          | Real provisioning: a gated Workflow runs a genuine `RunInstances` against floci (real instance _containers_, not a mock). **The BUILD half only** — corrected 2026-08-01: this demo declares no Source and no Connector, so nothing observes the instance back and the claim that "the Syncer observes the new instance appear" was never true here. `task e2e:live` found it by reporting a green run ending `ec2-instances: 0`. For the read half see the vSphere demo. **Not SSH-able** — see the fidelity note below.                                                                                                                                                                                                                                                                                                                     |
