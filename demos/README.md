...
| **[configure a network device](network-device/README.md)**                        | **here** (ansible + declared)            | FRR router (`network_cli`) | `real`                | **The estate that is not a server.** A target whose LOGIN SHELL is the device CLI — no POSIX shell, no python on the far end, no agent — so ansible's ordinary path cannot work and `connection.type: network_cli` is load-bearing rather than decorative. A declared device (nothing provisions a switch, so nothing observes its transport and the Step declares it), a brokered key like any other credential, and the change **read back out of the device's own running-config** rather than trusted from the Run. Closes ADR-0153's live half, which sat booked as "blocked on a target" until FRR turned out to be one. Building it FOUND that `network_cli` could never have connected: netcommon declares no python SSH library, so the collection installed no transport and the gate passed it anyway (ADR-0159). |
