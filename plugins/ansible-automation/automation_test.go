...
func TestHalvesOwnDisjointNamespaces(t *testing.T) {
	...
	if len(c) != 5 {
		t.Errorf("controller advertises %d contracts, want 5 (template/workflow/schedule/org/team)", len(c))
	}
	if len(n) != 4 {
		t.Errorf("content advertises %d contracts, want 4 (playbook/role/collection/inventory)", len(n))
	}
	for _, ns := range []string{"ansible.template", "ansible.workflow", "ansible.schedule", "ansible.org", "ansible.team"} {
		if !c[ns] {
