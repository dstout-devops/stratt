// Package puppetsim is a dev-harness stand-in for the PuppetDB/OpenVoxDB v4
...
// endpoint with real pagination (limit/offset + X-Records) so the puppet plugin
// runs its real enumeration/normalize code paths with no real server — the reason
...
// the mTLS client path is proven separately in the plugin's auth test.
package puppetsim
