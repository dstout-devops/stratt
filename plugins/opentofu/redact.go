...
// planned_values / prior_state module trees under sensitive_values, and output
// values flagged sensitive. The event stream is not a secret channel (§2.5) — and
// a content-blind core CANNOT redact, so redaction is the plugin/SDK's job
// (ADR-0047 §8). Unparseable documents pass through untouched (never worse than
// before) — tofu's own format is stable JSON, so that path is cold.
