...
task dev:vspheresim:proof     # create a VM, then observe it become reachable
```
...
| `-guest-image`     | `VSPHERESIM_GUEST_IMAGE`     | _(empty — backing disabled)_ | **Must run something long-lived.** A bare `alpine` starts, finds nothing to do, exits 0, and the VM then has a backing and no guest.                                                              |
