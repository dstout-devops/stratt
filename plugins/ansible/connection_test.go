...
func TestConnectionKeyComesFromTheCredentialMount(t *testing.T) {
	...
	}, nil, "/runner/known_hosts", false, oneKey, fakeStage)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnmountedConnectionRefNamesTheFix(t *testing.T) {
	_, err := connectionVars(&connectionParams{CredentialRef: "missing"}, nil, "", false, noMount, fakeStage)
...
func TestAmbiguousConnectionMountIsDiagnosed(t *testing.T) {
	_, err := connectionVars(&connectionParams{CredentialRef: "multi"}, nil, "", false, twoKeys, fakeStage)
...
func TestHostKeyCheckingDefaultsToVerifying(t *testing.T) {
	vars, err := connectionVars(&connectionParams{}, nil, "/runner/known_hosts", false, oneKey, fakeStage)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHostKeyCheckingOffIsExplicit(t *testing.T) {
	vars, err := connectionVars(&connectionParams{HostKeyChecking: HostKeyOff}, nil, "/runner/known_hosts", false, oneKey, fakeStage)
	if err != nil {
		t.Fatal(err)
	}
	args := vars["ansible_ssh_common_args"]
	...
	if _, err := connectionVars(&connectionParams{HostKeyChecking: "maybe"}, nil, "", false, oneKey, fakeStage); err == nil {
...
func TestNoConnectionBlockStillRuns(t *testing.T) {
	vars, err := connectionVars(nil, nil, "/runner/known_hosts", false, noMount, fakeStage)
	if err != nil {
...
func TestSingleKeyIsInferredThroughTheProjectedVolumeLayout(t *testing.T) {
	vars, err := connectionVars(&connectionParams{CredentialRef: "app-node-ssh"}, nil, "", false, projectedMount, fakeStage)
	if err != nil {
	...
	if _, err := connectionVars(&connectionParams{CredentialRef: "multi"}, nil, "", false, twoReal, fakeStage); err == nil {
