...
func TestProxyJumpRendersTheResolvedChain(t *testing.T) {
	vars, err := connectionVars(
		...
		"/runner/known_hosts", false, oneKey, fakeStage)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHopWithoutAddressIsRefused(t *testing.T) {
	_, err := connectionVars(&connectionParams{}, []Hop{{Name: "ghost"}}, "", false, oneKey, fakeStage)
...
func TestNoChainRendersNoProxyJump(t *testing.T) {
	vars, err := connectionVars(&connectionParams{User: "appops"}, nil, "/runner/known_hosts", false, oneKey, fakeStage)
	if err != nil {
		t.Fatal(err)
	}
