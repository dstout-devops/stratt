...
The single most important question for workflow security: **can an outside contributor trigger this
workflow, and if so, what token and secrets does it get?** GitHub answers this differently per
trigger.
...
The trap: `pull_request` from a fork is *safe* because GitHub deliberately strips the token down and
withholds secrets. Maintainers who find that "the secrets don't work on fork PRs" often switch to
`pull_request_target` to get them back — handing a write token and every secret to arbitrary
contributors.

## Why `pull_request_target` is dangerous

It checks out the **base** repo's workflow definition (so a fork can't change what runs), but runs
with full privileges. The danger is when the workflow then checks out the **fork's** code and
executes it:
...
- Labelling, commenting, or triaging on metadata is fine — just don't run the contributor's code.
- Prefer `pull_request` (safe read-only/no-secrets defaults) whenever possible.
