...
| Capability dependencies — plugins `requires` a capability *contract*, resolved first-class (the anti-Jenkins) | 🟡 framework | [ADR-0104](adr/0104-plugin-capability-dependencies.md); `types.ValidCapability` + `provides`/`requires` on both Kinds; registry resolves declared providers (store-visible, replica-consistent, health-independent), gates enablement, surfaces unmet/ambiguous as D6 **pending** (§1.8); ≥2-provider estate binding + the four enterprise providers (Temporal-spine, OpenBao, S3, EC2) are the follow-ons |
