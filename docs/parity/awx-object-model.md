...
| ------------------------------------------ | -------------------------------- | ------------------------------------------------------------------------------------------------ |
| **Object coverage** (which objects at all) | 🟢 **10 projected; the residue is 2 declines** (2026-08-05) | Spine + credentials + accounts + labels + EEs. `roles` (AWX-005, ADR-0130 D3) and `instance_groups` (AWX-008) are DECLINED with arguments, not pending — the object table below said 🟠 "nobody has looked" for both and contradicted this row until it was corrected. **The real open question is SHAPE, not coverage**: labels and EEs are derived from template payloads rather than enumerated, so one attached to nothing is invisible |
...
| `labels`                         | **projected** 🟢 (row was stale, corrected 2026-08-05) | ADR-0132 shipped it: `ansible.label` Entities reached by a `has-label` relation off the template. Note the SHAPE — they come from the template payload, not a `/labels/` enumeration, so a label attached to nothing is invisible. That is a real limit and a different one from "nobody has looked" | ~~AWX-006~~ |
| `execution_environments`         | **projected** 🟢 (row was stale, corrected 2026-08-05) | ADR-0133 shipped it: `ansible.executionenvironment` Entities reached by a `runs-in` relation. Same shape caveat as labels — derived from the template, not enumerated — so an EE no template references is invisible | ~~AWX-007~~ |
