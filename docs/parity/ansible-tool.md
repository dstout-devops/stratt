...
| **Content-root breadth** | 🟢 **nine artifact kinds** (row was stale, corrected 2026-08-04) | Roles, collections, playbooks, inventories, **var scopes**, **plugins**, **ansible.cfg**, **galaxy.yml**, and role **meta** — a shipped content root uses a role and nested `vars/<app>/<os_family>.yml`, verified through the real ConfigMap path. The row's named residue is closed: `meta/main.yml` is read (ANS-004, `rolemeta.go`) and so is the `roles:` half of `requirements.yml` — its own comment records that parsing only `collections:` had left role dependencies invisible |
| **Variable layout**      | 🟢 **projected** (row was stale, corrected 2026-08-04) | `VarScope` is a projected Kind: `group_vars/`/`host_vars/` in both the file and directory forms, with scope, the target group/host, and the TOP-LEVEL KEY NAMES — **never values** (§2.5). A vault-encrypted scope reports `vaulted: true` with an empty key list, because "binds nothing" and "binds things I cannot show you" must not render the same (§1.8). ANS-003/008 |
| **Custom content**       | 🟢 **projected** (row was stale, corrected 2026-08-04) | `Plugin` is a projected Kind across thirteen types — module, module_utils, filter, callback, action, lookup, test, vars, strategy, inventory, connection, cache, doc_fragments — in both repo layouts, and it records the owning ROLE when the file is role-local, because a role-local module travels with the role and is a different migration risk. ANS-006 |
...
| **Connection types**     | 🟡 **Windows is the residue** | ADR-0153 closed `network_cli`/`netconf` and all three credential forms; ADR-0156 added the OBSERVED substrate transports (`kubectl` live-proven 2026-08-01, `vmware_tools`/`aws_ssm` shipped but not). **Windows (`winrm`/`psrp`) remains unsupported and the enum REFUSES the value** — stated at estate-load rather than discovered at run time. This row read 🔴 "SSH-only, key-only … cannot be automated at all" until 2026-08-01, contradicting its own bottom line four rows below |
| **Vault**                | 🟢 **multi-identity ships** (row was stale, corrected 2026-08-04) | `params.vault` takes the v7 OBJECT or a v8 ARRAY; each entry with an `id` renders `--vault-id <id>@<path>`, each without keeps `--vault-password-file`. Pinned by a test that mixes all three (ANS-011, ADR-0153 D4). The object form is kept working deliberately: a Step cannot pin a Contract version, so an array-only read would have failed every shipped declaration the moment v8 landed |
...
| `roles/*/meta/main.yml` → `galaxy_info`                             | 🟢 author, license, min version, platform names     | Platform _versions_ are not projected — a supported-version matrix is not estate structure                                                                                                                                                                                                                           | ~~**ANS-004**~~ |
...
| `--syntax-check`, `--list-tasks`, `--list-hosts`           | 🟢 **shipped as a BUILD-time gate (2026-08-05)** | [ADR-0171](../adr/0171-a-file-a-step-calls-a-playbook.md): `task ansible:syntax` runs a REAL `ansible-playbook --syntax-check` as a subprocess in the EE image — the §3 boundary, not a violation of it — over every playbook the estate references, derived from each Actuator's `contentInputs`. Its own CI job, because `task ci` builds no EE. The row's "pre-flight gate before a Run" framing is REFUSED (D2): a dispatch-time check moves the failure into a Run already underway, and ADR-0117 D3a refused play parsing in dispatch. `--list-tasks`/`--list-hosts` are not shipped — they inform, they do not gate | ~~ANS-013~~ |
...
| `--module-path`                                            | 🟢 **not needed — proven 2026-08-04** | The row was justified by ANS-006 and conflated two things: projecting a module is not ansible LOADING it. There is no `modulePath` knob and no `ANSIBLE_LIBRARY`, and none is needed — the core mounts an Actuator's content into `project/`, so a repo's `library/` lands BESIDE the playbook, which is where ansible searches by default. `demos/network-device` ships a module in `library/`, a play calls it by name with nothing declaring a path, and the assertion is read off the Run's own log rather than inferred from a green status | ~~ANS-006~~     |
...
| Containers (`docker`/`podman`)             | ❌   | ❌ nobody has asked                                                                       |
| `kubectl`                                  | ❌   | ✅ — but **not as a declared type**: it is an OBSERVED transport, see below (ADR-0156 D5) |
...
| ------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `kubernetes.core.kubectl`       | `kubectl exec`                                       | **nothing in the guest — but a kubeconfig on the CONTROL NODE** | 🟢 **LIVE-PROVEN 2026-08-01** — `demo:region-to-cert` (Apache on a kubecompute-built pod over `kubectl exec` with a brokered kubeconfig, `rc=0`, served off the wire on :8080) and `demo:scale-fleet` (one Assignment converging THREE pods the same way) (ADR-0156 D4a) |
...
> **This row read 🟢 LIVE-PROVEN from the day the transport shipped until 2026-08-01, and it was
> false.** Nothing had ever converged over it. The evidence it cited — _"a pod asserted to have no
> sshd, no ssh client and nothing on port 22"_ — describes an assertion that exists nowhere in the
> repo, against a pod that does not exist: `kubecompute` bakes `openssh` into every host it builds,
> and sshd is measurably running in them. What was actually proven was that `mgmt.transport` is
> **observed**; `demos/scale-fleet` asserted the Facet and narrated it as a converge it never ran.
>
> It took a real converge to find out. The transport could not work at all: execution pods are
> spawned with `AutomountServiceAccountToken: false` — "the pod has no cluster identity" — so
> `kubectl exec` had no credential, and `pods/exec` was granted nowhere in the repo. See ADR-0156
> D4a. **The pods it now converges still have sshd; the converge simply does not use it.** Removing
> that bootstrap is the coupling this transport makes removable, and it is booked, not done — which
> is the honest version of what this row used to claim.
...
honestly observe _neither_ EC2 path. `KeyName` means a key is AUTHORIZED, not that sshd is listening
