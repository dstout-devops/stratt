...
| `--module-path`                                            | 🟠     | **The row's reasoning no longer holds** (2026-08-04): ANS-006 ships, so a repo's own modules ARE projected — but projecting that a module exists is not the same as ansible loading it, and this row conflated them. No `modulePath` knob and no `ANSIBLE_LIBRARY` exist. What is UNVERIFIED is whether ansible's automatic `library/`-beside-the-playbook discovery already covers it given the dispatcher's `/runner/<path>` mount layout: plausible, untested, and not claimed either way until a Run proves it | **ANS-006**     |
