...
1. **K8s + Helm collector** — the dimension's slice 1, split so each half is verifiable:
   - **1a (SHIPPED):** the `service.endpoint` + `software.chart` facet schemas and the `kubeservices` plugin **normalizer** (content-expertise, client-go-free, fixture-tested): a K8s Service → a `service` Entity (`service.endpoint`); a Helm release → an `application` Entity (`software.chart`) + the `provides` M:N edge to its services, **derived from the app.kubernetes.io Helm labels**. The "chart CVEs for free" claim is **proven** — `software.chart` is the `{charts:[{name,version,…}]}` component shape, so `CheckSoftwareAdvisories`'s `software.%` pass raises a Finding on a vulnerable chart with zero check change.
   - **1b (SHIPPED):** the live plugin server (`kubeservices.Server`: client-go lists Services, `fromCoreV1` maps `corev1.Service` → the normalizer shape; in-cluster/KUBECONFIG client; fake-clientset tested) + the **host per-source full-sync delete-and-replace** of `provides` edges (MF-2): `Projector.RetractSourceRelationsExcept` + `RelationTypesBySource`, wired at the pluginhost `FullSyncComplete` boundary, scoped to the Source (never another's edges), sweeping every relation type re-emitted **or** previously owned. **INV-1 proven structurally** (a raw `service.endpoint`/`provides` write bypassing the write-path is rejected by the `enforce_write_path`/`relation_write_path` triggers). The `provides` edge reaches the graph only here, so MF-2 lands exactly when needed.
2. **`depends-on`** — *after* relation tombstone/GC (ADR-0059 gap): a mesh/declared dependency source.
3. **service ↔ identity — SHIPPED.** kubeservices projects each service's K8s DNS name as a `dns.fqdn` identity, and `CorrelateIdentities` links a cert → the `service` it attests when the cert's CN or SAN matches that FQDN — the identity↔service cross-dimension edge (`cert --identifies--> service --provides<-- application(chart)`). Services carry no `identity.subject` (§2.1: scim-owned), so the DNS identity is the match key; no leaver Finding for a service (it is not a disabled user).
