...
1. **K8s + Helm collector** — the dimension's slice 1, split so each half is verifiable:
   ...
   - **1b:** the live plugin server (client-go lists Services, maps `corev1.Service` → the normalizer shape) + the **host per-source full-sync delete-and-replace** of `provides` edges (MF-2 — the `provides` edge reaches the graph only here, so MF-2 lands exactly when needed) + the INV-1 test (no write outside the collector, facets + edge). Single §2.1 owner. Final identifiers re-linted via vocabulary-linter at merge.
