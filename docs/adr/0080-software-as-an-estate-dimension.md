...
- **Follow-ups (slices):** (2, **shipped**) the declarable advisory ruleset (`advisories/*.yaml`, compliance-as-data) + reconcile-cadence wiring of the check — the live patch loop, dormant until a collector projects inventory; (2b) a real OS-package collector projection (the single §2.1 `software.package` write-owner) — needs SoR/EE access; (3) other delivery forms (`software.container`/`software.chart`); (4) the service/capability dimension + `provides`/`depends-on` typed edges (its own ADR, grounded in a registry/mesh connector); relation tombstone/GC before the dependency graph (ADR-0059 gap).
