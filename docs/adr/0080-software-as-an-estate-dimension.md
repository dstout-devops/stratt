...
- **Follow-ups (slices):** (2) a real OS-package collector projection + a declarable advisory source + reconcile-cadence wiring; (3) other delivery forms (container/Helm); (4) the service/capability dimension + `provides`/`depends-on` typed edges (its own ADR, grounded in a registry/mesh connector); relation tombstone/GC before the dependency graph (ADR-0059 gap).
