...
- **Follow-ups:** ~~§7.4b — the estate admission-policy surface~~ and ~~§7.4c — the loader wiring~~ **both landed:** `estate/admission/*.yaml` declares the policy (parsed to `Declarations.AdmissionControls`, validated at load), and `ParseDir(root, decider)` admits every other manifest through the port — a `deny` rejects the load fail-closed (§1.8), the Decider injected via the reconcile `Controller` and the `stratt` CLI (nil ⇒ admission skipped, e.g. the boot authz-only load). Remaining: the Kyverno-JSON/OPA admission provider over the sovereign gRPC port; a port `ValidateAdmission` for engine-selected admission-dialect validation; richer `match`-by-kind ergonomics so authors needn't hand-guard with `has()`.
