...
- **Status:** Superseded-in-part by [ADR-0086](0086-adopt-per-object-in-place.md) — Decision #1 (the
  ...
  `adopt`; Decision #2's transform _input_ re-homes to a targeted per-object read. Decision #2 mapping
...
> therefore **`stratt adopt`** — taking authority over an _already-observed_ object, flipping
...
Flow 6 calls the importer "an AWX Syncer + transform," but its _outputs_ are
Git-declared **desired state** (Views / Workflows / CredentialRefs), not the
projection graph (§1.2). So it is a hybrid: it reuses the Connector _read_
...
An AWX job template references its playbook by SCM _project + path_ — AWX's API
...
2. **Transform — `awximport/` → a reviewable CaC bundle.** Pure (I/O only in
   ...
   - **inventory → View** (the crux, §1.2). _Dynamic_ (aws_ec2/vmware/azure_rm)
     ...
     hosts. _Smart_ (`host_filter`) → reduce simple `and`-conjunctions of
     equality into selector predicates; drop and **report** the irreducible
     remainder (or/not/regex/non-exact operators). _Static_ (manual hosts) — the
...
- **Deferred (flagged, not faked):**
  - ~~**Survey enforcement.**~~ — **closed.** The deferral was correct about the
    cause: `types.Step` could reference no input Contract, so the survey was
    emitted as a detached `contracts/<name>.survey.schema.json` and enforced by
    nothing. **ADR-0118 D2 built the missing seam** — a Workflow declares its
    launch interface as a JSON Schema document, validated at one chokepoint
    below all four launch paths — so the survey now lands as
    `Workflow.inputs` and the detached copy is **not** also emitted (two
    authorities for one fact, §1.2). Each answer is bound into the Step as
    `extraVars: {var: "{{.launch.var}}"}`, which `ansible.input.v5` already
    documents as "the landing field for AWX launch extra_vars and survey
    answers" — otherwise the import would have shipped a launch interface that
    was declared, validated, and then silently dropped. Proven by the golden
    bundle: rebinding one answer to an undeclared name fails
    `TestAdoptGoldenBundleParses` naming the input and the declared set.
    Note the binding is **not** the `inputContract`-on-Step design this entry
    predicted. A survey is a **launch** interface, and a Step in the middle of a
    DAG has no launch boundary — which is why an imported workflow-job-template
    node whose job template has a survey now carries a report note saying its
    answers are not imported, rather than a binding that could not work.
  - **One §2.5 defect fell out of enforcing it.** A `password` survey question
    used to be typed as a string marked `x-stratt-sensitive` — harmless while the
    document was read by nothing, a real violation the moment it became a live
    launch input, since the material would ride the Run, the audit stream, and
    `--extra-vars`, whose own Contract forbids it. Password questions are now
    **refused, not typed**: not imported as inputs at all, with a blocking report
    entry demanding a CredentialRef, exactly as an imported AWX credential blocks.
...
page/page_size/filters, run→job status, SSE→stdout adapter); private-repo SCM
credential-helper + git-server harness e2e; notifications; Intent kinds GA +
object-locked Evidence (Phase 3). **Survey enforcement is no longer on this list** —
it landed as `Workflow.inputs` (ADR-0118 D2), not as the Step `inputContract` binding
predicted here; see _Deferred_ above.
