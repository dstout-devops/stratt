...
**This section stated a question. It has since been answered: [ADR-0152](adr/0152-a-facet-claim-is-qualified.md)**
(Proposed) — the claim grain gains a **qualifier**, derived at compile, and so does the Facet PK.
The rest of this section is kept as written, because it is the statement of the problem the ADR
argues from and three of its paragraphs were adopted verbatim as constraints. Two things below are
now stale and are corrected inline: the word, and `svc-fleet`.
...
ADR~~ — **done: the word is `qualifier`** (ADR-0152 D1). Doing it in this order paid: the obvious
candidates were both taken in ways a grep for the bare word does not show. `instance` is a shipped
Facet namespace prefix meaning facts about an EC2 machine (`instance.compute`/`.network`/`.state`,
ADR-0095) as well as `provision.Instance`; and `slot` — the word that best carries "the thing that
is scarce" — is used across the provisioning ADRs (0059, 0110, 0113, 0120) for the place a
capability PROVIDER is bound into, so to a reader of those it says _which provider_.
...
**Estate consequence, if the fix lands:** ~~`svc-fleet` / `view:svc-servers` exist only to work
around this limit~~ — **stale, and the correction matters.** Those two were reverted in `3dcd66a`
(register item 12), so there is nothing to delete. The live workaround is a whole second managed
NODE: `estate/hosts/managed-tomcat.yaml` + `estate/views/managed-tomcat.yaml` exist because one host
holds one application, and all three of `managed-app`/`managed-tomcat`'s files say so in their own
comments. ADR-0152 D9 makes retiring that node the acceptance test — the realistic topology is
apache reverse-proxying tomcat on one host.
...
| #   | Seam                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Layer        | Status                                    |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ----------------------------------------- |
| 1   | ~~Facet/claim grain has no instance key (§6)~~ — **decided**: [ADR-0152](adr/0152-a-facet-claim-is-qualified.md) adds a `qualifier` to the claim key AND to the Facet PK, derived at compile from the resolved spec. Keying only the claim was rejected explicitly: it moves the collision from compile time to Run execution order and hides it. The port does NOT change (core stamps the qualifier from the claim). Charter review falsified two of the draft's claims: the fail-closed scalar read does not exist at `mgmt.address`/`mgmt.site`, so the suppression now carries its own Finding; and what §2 actually moves is **§2.1's "a _named_ fragment"**, not §2.4 — an amendment is proposed there and awaits an explicit steward decision. **Expand shipped** (migration 00047 + the claim key + every read path): a declared `observe.qualifier` is REFUSED at load until the contract release folds the column into `facet_pkey`, because half the widening is a silent last-writer-wins (D10) | L2 / L4      | **EXPAND SHIPPED** — contract release owed |
