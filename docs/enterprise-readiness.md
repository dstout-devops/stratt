...
| **SEC-2** | Dev trusted-header = one-flag full auth bypass, no structural guard (can be enabled in prod alongside OIDC; attacker omits the Bearer, sets `X-Stratt-Principal: admin`) | Serious | 🔴 | `main.go:290`; `server.go:366-371` | Refuse boot when `devPrincipal && (oidcIssuer!="" \|\| env==production)` — mirror the audience hard-fail |
