...
func checkNestedWorkflows(decls Declarations) error {
	...
	for _, w := range decls.Workflows {
		for _, s := range w.Steps {
			...
			child, ok := byName[s.Workflow]
			if !ok {
				return fmt.Errorf("%s runs workflow %q, which is not declared — the Step would fail at "+
					"launch inside a half-run tree, so it is refused here (§1.8)", what, s.Workflow)
			}
			// The child's own launch interface, applied at the moment the reference is written.
			// RunDAG re-applies it at launch through ResolveLaunchInputs (the one chokepoint,
			// ADR-0118 D4); this is that same rule moved to the diff.
	...
	return checkNestingGraph(byName)
}
...
func checkNestingGraph(byName map[string]types.Workflow) error {
	const (
		unvisited = 0
		onStack   = 1
		done      = 2
	...
	walk = func(name string) error {
		...
		for _, s := range w.Steps {
			if s.Workflow != "" {
				children = append(children, s.Workflow)
			}
		}
