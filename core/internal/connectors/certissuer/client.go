...
// OpenBao). We deliberately do not vendor an official SDK — the surface is
// four calls and staying hand-rolled keeps the module graph boring (§1.4) and
// the Connector contract sovereign over the transport (§1.5). The same shape
// serves step-ca/Vault/cert-manager behind the cert-issuer contract later.
type Client struct {
	addr  string // e.g. http://localhost:8200
	token string // X-Vault-Token; read-side projection credential (§2.5)
...
// issueData is the shape of an issue response (POST .../issue/:role).
...
// and any revoked certs — the Normalizer filters those.
...
// Issued is the result of issuing a leaf certificate.
type Issued struct {
	Serial string
	PEM    string
}
...
// TTL (e.g. "720h"). Used by the cert-issuer Actuator's issue/renew ops.
func (c *Client) Issue(ctx context.Context, role, commonName, ttl string) (Issued, error) {
	...
	return Issued{Serial: d.SerialNumber, PEM: d.Certificate}, nil
}

// Revoke revokes a certificate by serial (colon-hex).
func (c *Client) Revoke(ctx context.Context, serial string) error {
	_, err := c.do(ctx, http.MethodPost, "/revoke",
