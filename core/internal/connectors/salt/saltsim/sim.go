...
// Connector's, never this). It serves just enough of the rest_cherrypy API for
// the salt Syncer and Emitter to run their real code paths with no real Salt
// master — eauth login + a token gate, the runner cache.grains enumeration, and
// (for the Emitter) the /events SSE stream. The reason harnesses are first-class
// for this out-of-network OSS build.
package saltsim
...
type Sim struct {
	...
	eventCh chan event                // queued SSE frames (Emitter tests)
}
...
func New() *Sim {
	return &Sim{grains: map[string]map[string]any{}, eventCh: make(chan event, 64)}
}
...
// Handler serves login, the runner enumeration, the SSE event stream, and hooks.
func (s *Sim) Handler() http.Handler {
	...
	mux.HandleFunc("POST /_sim/event", s.simEvent)
	return mux
}
...
// events streams queued events as SSE (tag:/data: frames), then holds the
// connection until the request context ends (the client cancels).
func (s *Sim) events(w http.ResponseWriter, r *http.Request) {
	if !s.authed(r) {
		http.Error(w, `{"error":"unauthorized"}`, http.StatusUnauthorized)
		return
	}
...
// EmitEvent queues one Salt event for the /events stream (also via POST
// /_sim/event) so Emitter tests can drive real translation.
func (s *Sim) EmitEvent(tag string, data map[string]any) {
	s.eventCh <- event{Tag: tag, Data: data}
}

func (s *Sim) simEvent(w http.ResponseWriter, r *http.Request) {
	var body struct {
		Tag  string         `json:"tag"`
	...
	s.EmitEvent(body.Tag, body.Data)
	w.WriteHeader(http.StatusNoContent)
}
