...
func TestPinsAreStable(t *testing.T) {
	...
	// ansible.input v9 (the kubectl REACH CREDENTIAL, ADR-0156 D4a — the half its transport table
	// omitted by asking only what the guest needs) resolves as the current version; v1–v8 stay
	// pinned alongside it (every version keeps its own pin row — only the
	...
	if estateVersions["actuators/ansible.input"] != 9 {
